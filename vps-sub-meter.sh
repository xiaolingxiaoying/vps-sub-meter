...
#
# 功能:
#   - 固定版本 sing-box-yg 核心节点安装与五协议节点生成
...
# ===================== sing-box-yg 核心集成 =====================
has_singbox_yg_core() {
    [ -s /etc/s-box/sb.json ] && [ -x /etc/s-box/sing-box ]
}

has_subscription_exports() {
    [ -s /etc/s-box/clmi.yaml ] && [ -s /etc/s-box/jhsub.txt ] || return 1
    grep -q '[^[:space:]]' /etc/s-box/jhsub.txt || return 1
    python3 - /etc/s-box/sbox.json <<'PYEOF' >/dev/null 2>&1
import json, sys
...
fetch_pinned_singbox_yg() {
    mkdir -p /usr/local/lib/vps-sub-meter
    if [ ! -s "$UPSTREAM_SINGBOX_YG_SCRIPT" ]; then
        echo "=> 下载固定 sing-box-yg 快照: ${UPSTREAM_SINGBOX_YG_COMMIT:0:12}"
        apt-get update -qq
        apt-get install -y -qq curl ca-certificates
        curl -fsSL "$UPSTREAM_SINGBOX_YG_URL" -o "$UPSTREAM_SINGBOX_YG_SCRIPT"
        # 上游菜单中的“更新”也只能回到同一固定快照，不能静默跳转到 main。
        sed -i \
            -e "s#https://raw.githubusercontent.com/yonggekkk/sing-box-yg/main/sb.sh#$UPSTREAM_SINGBOX_YG_URL#g" \
            -e "s#https://raw.githubusercontent.com/yonggekkk/sing-box-yg/main/version#https://raw.githubusercontent.com/yonggekkk/sing-box-yg/${UPSTREAM_SINGBOX_YG_COMMIT}/version#g" \
            "$UPSTREAM_SINGBOX_YG_SCRIPT"
        chmod 700 "$UPSTREAM_SINGBOX_YG_SCRIPT"
    fi
}
...
write_upstream_manifest() {
    mkdir -p /etc/sub-srv
    cat > "$UPSTREAM_MANIFEST" <<EOF
# Managed by vps-sub-meter.sh
UPSTREAM_NAME='yonggekkk/sing-box-yg'
UPSTREAM_COMMIT='$UPSTREAM_SINGBOX_YG_COMMIT'
UPSTREAM_SCRIPT='$UPSTREAM_SINGBOX_YG_SCRIPT'
CORE_CONFIG='/etc/s-box/sb.json'
...
backup_existing_singbox_yg() {
    local backup_dir
    if [ "$MIGRATION_BACKUP_DONE" = true ]; then
        return
    fi
    backup_dir="/var/backups/vps-sub-meter/sing-box-yg-$(date +%Y%m%d_%H%M%S)"
    mkdir -p "$backup_dir"
...
switch_singbox_outbound() {
    local config="/etc/s-box/sb.json" backup="/etc/s-box/sb.json.bak" tmp choice
    if [ ! -s "$config" ]; then
        echo "错误: 未找到 $config"
        return 1
    fi
...
choose_integrated_mode() {
    echo "=================================================="
    echo " VPS 节点订阅 + 流量统计 一体化管理"
    echo "=================================================="
    echo "  1) 全新部署：安装 sing-box 五协议节点 + 订阅流量服务"
    echo "  2) 导入现有 sing-box-yg：保留节点/证书，仅部署订阅流量服务"
    ...
    case "$integrated_mode" in
        1)
            if has_singbox_yg_core; then
                backup_existing_singbox_yg
                echo "=> 检测到现有 sing-box-yg，将保留现有节点并进入导入模式"
            else
                run_singbox_yg_core
            fi
            ;;
        2)
            if ! has_singbox_yg_core; then
                echo "错误: 未检测到 /etc/s-box/sb.json 和 sing-box 核心；请选择全新部署"
                exit 1
            fi
            backup_existing_singbox_yg
            echo "=> 已检测到现有 sing-box-yg，节点和证书将保持不变"
            ;;
        3)
            run_singbox_yg_core
            if ! has_singbox_yg_core; then
                echo "错误: 节点核心尚未安装完成，请重新运行一体化脚本"
                exit 1
            fi
            ;;
        4)
            if ! has_singbox_yg_core; then
                echo "错误: 仅更新模式需要已有 sing-box-yg 节点核心"
                exit 1
            fi
            ;;
        5)
    ...
    if ! has_subscription_exports; then
        echo "=> 节点导出文件尚未齐全，尝试通过固定核心菜单生成"
        run_singbox_yg_core
    fi
    if ! has_subscription_exports; then
        echo "错误: 未生成 clmi.yaml、sbox.json、jhsub.txt，无法创建订阅服务"
        exit 1
    fi
    write_upstream_manifest
}
...
if [ -f /etc/s-box/clmi.yaml ]; then
    cp -f /etc/s-box/clmi.yaml /var/lib/subsrv/client.yaml
else
    echo "# 暂无订阅内容，等待 yonggekkk 脚本生成" > /var/lib/subsrv/client.yaml
    echo "=> 警告: /etc/s-box/clmi.yaml 不存在，已创建默认空配置"
fi
...
if [ -f /etc/s-box/sbox.json ]; then
    ...
    cp -f /etc/s-box/sbox.json "$TMP_JSON"
